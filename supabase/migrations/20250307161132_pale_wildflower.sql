/*
  # BCDR Overview Analysis Function
  
  1. New Types
    - Creates composite types for recommendations and overview stats
    - Handles existing type gracefully
  
  2. New Function
    - Creates function to calculate BCDR overview statistics
    - Includes comprehensive metrics across all assessment types
    - Handles trends and comparisons
...
DO $$ 
BEGIN
  IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'overview_recommendation') THEN
    DROP TYPE public.overview_recommendation CASCADE;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'bcdr_overview_stats') THEN
    DROP TYPE public.bcdr_overview_stats CASCADE;
  END IF;
END $$;
...
BEGIN
  ...
  SELECT COUNT(DISTINCT da.department_id) INTO assessed_departments
  ...
  WITH prev_completion AS (
    SELECT COUNT(DISTINCT da.department_id)::numeric / NULLIF(total_departments, 0)::numeric * 100 as completion_rate
    FROM department_assessments da
    JOIN bcdr_assessments ba ON ba.id = da.assessment_id
    WHERE ba.organization_id = org_id
    AND da.status = 'completed'
  ...
  SELECT completion_rate INTO prev_department_completion
  FROM prev_completion;
  ...
  WITH evidence_stats AS (
    ...
    FROM (
      SELECT rr.evidence_links, rq.evidence_required 
      ...
      WHERE ba.organization_id = org_id
      UNION ALL
      SELECT gar.evidence_links, gaq.evidence_required 
      FROM gap_analysis_responses gar
      JOIN gap_analysis_questions gaq ON gaq.id = gar.question_id
      JOIN bcdr_assessments ba ON ba.id = gar.assessment_id
      WHERE ba.organization_id = org_id
      UNION ALL
      SELECT mar.evidence_links, maq.evidence_required 
      FROM maturity_assessment_responses mar
