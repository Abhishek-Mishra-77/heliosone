...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE indexname = 'idx_resiliency_questions_category'
  ) THEN
    CREATE INDEX idx_resiliency_questions_category ON resiliency_questions(category_id);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE indexname = 'idx_resiliency_responses_assessment'
  ) THEN
    CREATE INDEX idx_resiliency_responses_assessment ON resiliency_responses(assessment_id);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE indexname = 'idx_resiliency_questions_conditional_logic'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_resiliency_categories_updated_at'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_resiliency_questions_updated_at'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_resiliency_responses_updated_at'
  ) THEN
