...
DO $$ 
BEGIN
  -- Add created_by to bcdr_assessments if it doesn't exist
  IF NOT EXISTS (
  ...
  -- Add alternative_procedures to business_processes if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
  ...
  -- Add assessment_id to business_processes if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'business_processes' 
  ...
  -- Add index for assessment_id if it doesn't exist
  IF NOT EXISTS (
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
    WHERE indexname = 'idx_bcdr_assessments_created_by'
  ) THEN
    CREATE INDEX idx_bcdr_assessments_created_by 
    ON bcdr_assessments(created_by);
  END IF;
