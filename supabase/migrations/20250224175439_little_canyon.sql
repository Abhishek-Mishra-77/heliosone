...
-- Get Matthew's IT department ID and create fresh assignments
WITH matthew_it_dept AS (
...
FROM matthew_it_dept d
CROSS JOIN department_questionnaire_templates t
CROSS JOIN admin_user
WHERE t.department_type = d.department_type
...
DROP INDEX IF EXISTS idx_active_assignments_unique;

-- Create new indexes for better performance
CREATE INDEX idx_dept_questionnaire_dept_template 
ON department_questionnaire_assignments(department_id, template_id);

CREATE INDEX idx_dept_questionnaire_status 
