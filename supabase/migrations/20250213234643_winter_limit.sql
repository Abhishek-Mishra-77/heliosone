...
CREATE POLICY "allow_organization_members" ON departments
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM users
...
CREATE POLICY "allow_organization_members" ON department_users
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "allow_organization_members" ON department_assessments
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM departments d
      JOIN users u ON u.organization_id = d.organization_id
      WHERE u.id = auth.uid()
      AND d.id = department_assessments.department_id
    )
  );

CREATE POLICY "allow_admins_create_assessments" ON department_assessments
